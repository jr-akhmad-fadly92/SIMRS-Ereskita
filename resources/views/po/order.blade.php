...
@section('content')
					...
					@if(isset($po_id))
						<form id="formAdd" method="post" class="form-horizontal">
							...
							<div class="col-sm-12 no-padding">
									...
									<button type="button" id="cariItemPilihan" class="btn btn-success btn-flat">
										<i class="fa fa-search"> </i> Stok Habis
									</button>
							</div>
							<div class="col-sm-12 no-padding">
		...
		<div class="box box-primary">
			...
			<div class="box-body">
				<div class="">
					...
					<div class="col-sm-12 no-padding">
						<a href="{{ url('po-kirim-order/'.$po_id) }}" class="btn btn-success btn-flat pull-right" onclick="return confirm('Yakin transaksi diselesaikan?')">Kirim Order</a>
					</div>
				</div>
			</div>
		</div>
	...
	<div class="modal fade" id="addItemPilihan" tabindex="-2" role="dialog" aria-labelledby="" aria-hidden="true">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					<h4 class="modal-title-pilihan" >Pilih Obat yang Stok Habis / Limit</h4>
				</div>
				<div class="modal-body">
					<div class='table-responsive'>
						<table id="obatpilihan1" class='table table-striped table-bordered table-hover table-condensed'>
							<thead>
								<tr>
									<th>Kode</th>
									<th>Nama</th>
...
//OPEN MODAL OBAT Pilihan
$('#cariItemPilihan').on('click', function() {
	$('#addItemPilihan').modal('show');
	$('.modal-title-pilihan').text('Tambah Obat Stok Limit');
	if($('.modal-body tbody').html()==''){
		$('#obatpilihan1').DataTable({
			destroy: true,
			autoWidth: false,
			processing: true,
			serverSide: true,
			ajax: '/po-masterobat-pilihan',
			columns: [
				{data: 'kode'},
				{data: 'nama'},
				{data: 'stok'},
...
$(document).on('click', '.insert-pilihan',function () {
	$('input[name="kode_item"]').val($(this).attr('data-kode'));
	$('input[name="nama_item"]').val($(this).attr('data-nama'));
	$('input[name="satuan"]').val($(this).attr('data-satuan'));
	
	$('#addItemPilihan').modal('hide');
});
