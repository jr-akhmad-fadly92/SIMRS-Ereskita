...
@section('content')
    <div class="box box-primary">
		<div class="box-header with-border">
			<h3 class="box-title">
				Order Non Medis {{ ucfirst(strtolower(Auth::user()->role()->first()->display_name)) }}
			</h3>
		</div>
		<div class="box-body">
			<div class="row">
				<div class="col-md-6">
					@if(!isset($po_id))
						{!! Form::open(['method' => 'POST', 'url' => 'depo-nonmedis/order', 'class' => 'form-horizontal']) !!}				  
							...
							<div class="btn-groups pull-right">
								<a href="{{ url('depo-nonmedis') }}" class="btn btn-success btn-flat">BATAL</a>
					...
					@if(isset($po_id))
						<form id="formAdd" method="post" class="form-horizontal">
							...
							<div class="col-sm-12 no-padding">
								...
								<input type="text" name="satuan" class="form-control" readonly="true">
		...
		<div class="box box-primary">
			...
			<div class="box-body">
				<div class="">
					...
					<div class="col-sm-12 no-padding">
						<a href="{{ url('depo-kirim-order-nonmedis/'.$po_id) }}" class="btn btn-success btn-flat pull-right" onclick="return confirm('Yakin transaksi diselesaikan?')">Kirim Order</a>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class='table-responsive'>
						<table id="masterNM" class='table table-striped table-bordered table-hover table-condensed'>
							<thead>
								<tr>
									<th>Nama</th>
...
<script type="text/javascript">
	...
	$('#cariItem').on('click', function() {
		$('#addItem').modal('show');
		$('.modal-title').text('Tambah Non-Medis');
		$('#masterNM').DataTable().destroy();
		$('#masterNM').DataTable({
			
			autoWidth: false,
			processing: true,
			serverSide: true,
			ajax: '/depo-master-nonmedis',
			columns: [
					{data: 'nama_barang'},
					{data: 'jumlah'},
...
$(document).on('click', '.hapus', function(e) {
	...
	if (confirm('Apakah yakin item ini akan dihapus?')) {
		$.ajax({
			url: '/depo-hapus-detail-nonmedis/' + id,
			type: 'GET',
			success: function (data) {
				console.log(data);
				if(data.sukses == true) {
