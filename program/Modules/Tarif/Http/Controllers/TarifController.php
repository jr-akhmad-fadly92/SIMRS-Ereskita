...
class TarifController extends Controller
{
	public function index(){
			if(strtolower(Auth::user()->role()->first()->name)=='administrator'){
				...
				$data['jasa_medis']		= Jasa_medis::where('id',1)->first();
			}else{
	...
	public function update_jasa_medis(Request $request)
	{
		Jasa_medis::where('id',1)->update([
			'dokter'=>$request['jasa_dokter'],
			'perawat'=>$request['jasa_perawat']
		]);
		$jasa = Jasa_medis::where('id',1)->first();
		return response()->json(['sukses'=>true,'jasa_dokter'=>$jasa->dokter,'jasa_perawat'=>$jasa->perawat]);
	}
	...
	public function store(SavetarifRequest $request)
	{
		if(strtolower(Auth::user()->role()->first()->name)=='administrator'){
			...
			{ if($request['kategoritarif_id']=='7'||$request['kategoritarif_id']=='8'||$request['kategoritarif_id']=='9'){
				$tarif = new Tarif();
				...
				$tarif->mapping_pemeriksaan = $request['mapping_pemeriksaan'];
				$tarif->tarif_kelas_rj = $request['managemen'] + $request['tarif_tindakandr'] + $request['tarif_tindakanpr'];
				$kategori_tarif= Kategoritarif::where('id',$request['kategoritarif_id'])->first();
				$tarif->kategoriheader_id = $kategori_tarif->kategoriheader_id;
