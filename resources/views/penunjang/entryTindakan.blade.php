	...
	<div class="col-md-2 no-padding" style="padding-right:10px!important;">	
		<h4 style="margin-top:0;font-size:14px;font-weight:600;">
			@if (substr($jenis->status_reg,0,1) == 'G')
				Penata Jasa {{ Auth::user()->role()->first()->display_name }} - Rawat Darurat
			@elseif (substr($jenis->status_reg,0,1) == 'J')
				Penata Jasa {{ Auth::user()->role()->first()->display_name }} - Rawat Jalan
			@elseif (substr($jenis->status_reg,0,1) == 'I')
				Penata Jasa {{ Auth::user()->role()->first()->display_name }} - Rawat Inap
			@else
				Penata Jasa {{ Auth::user()->role()->first()->display_name }} - Tindakan Langsung
			@endif
		</h4>
				...
				<div class="pull-right" style="width: 40%;">
					<div class="col-md-12 col-sm-12 col-xs-12 no-padding">
						...
						<div class="col-md-8 col-sm-8 col-xs-8" style="margin-top:3px;">
							{!! Form::select('pelaksana', $perawat, null, ['class' => 'form-control select2', 'placeholder'=>'-- pilih pelaksana --', 'style'=>'width:100%;', 'onchange'=>'updatePelaksana(this.value)']) !!}
						</div>
						<div class="col-md-4 col-sm-4 col-xs-4" style="padding-left:0;margin-top:5px;">
							<a href="{{ url('penunjang/selesai/'.$reg_id) }}" class="btn btn-primary btn-flat btn-block">
						...
						<div style="padding:0 10px;">
							<div class="col-md-12">
								<div class="col-md-4 no-padding">
									<div class="box-body" style="background:#f9f9f9;margin:0 10px 10px 0;">
									...
									<div>
										<table class='table table-striped table-bordered table-hover table-condensed'>
											<thead>
												...
												<th>Waktu</th>
												<th>Status</th>
												@role(['supervisor','fisioterapi','kamarbersalin','administrator'])
												<th>Hapus</th>
												@endrole
												</tr>
											</thead>
											<tbody>
												...
												<tr>
													...
													<td>
														@php
															if(strtolower(Auth::user()->role()->first()->name)=='fisioterapi'){
																echo baca_dokter($d->fisioterapi);
															}
														@endphp
													</td>
													<td>{{ $d->user->name }}</td>
													...
													@else
														<a href="{{ url('penunjang/hapus-tindakan/'.$penunjang.'/'.$d->id.'/'.$d->registrasi_id.'/'.$d->pasien_id) }}" onclick="return confirm('Yakin akan di hapus?')" class="btn btn-danger btn-sm btn-flat"><i class="fa fa-trash-o"></i></a>
													@endif
													</td>
													@endrole
												</tr>
...
function updatePelaksana(val){
	$.ajax({
		...
		type: 'POST',
		url: '/penunjang/update-pelaksana',
		data: {pelaksana: val, registrasi_id: $("#reg_id").val()},
...
$.ajax({
	...
	success: function(data){
		$.each(data, function(index, val) {
			$('select[name="masterobat_id"]').append('<option value="'+val.id+'">'+val.nama+' | Rp '+ribuan(mojs('{{substr($jenis->status_reg,0,1)}}',val.hargajual))+'</option>');
		});       
	}
});
