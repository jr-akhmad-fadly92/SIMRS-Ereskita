              ...
              @if (isset($dokter))
                @foreach ($dokter as $d)
                  <tr>
                    <td>{{ $no++ }}</td>
                    <td>{{ $d->nama }}</td>
                    <td class="text-center"><a href="{{ url('/direksi/detail-kinerja-rawat-jalan/'.$d->id.'/'.$cara_bayar_id.'/PM') }}" target="_blank">{{ number_format( pemeriksaan($d->id, 'TA', $tga, $tgb, $cara_bayar_id) ) }}</a></td>
                    <td class="text-center"><a href="{{ url('/direksi/detail-kinerja-rawat-jalan/'.$d->id.'/'.$cara_bayar_id.'/KS') }}" target="_blank">{{ number_format( konsultasi($d->id, 'TA', $tga, $tgb, $cara_bayar_id) ) }}</a></td>
                    <td class="text-center"><a href="{{ url('/direksi/detail-kinerja-rawat-jalan/'.$d->id.'/'.$cara_bayar_id.'/TN') }}" target="_blank">{{ number_format( tindakan($d->id, 'TA', $tga, $tgb, $cara_bayar_id) ) }}</a></td>
                    {{-- @foreach ($klinik as $r)
                      <td></td>
                      <td class="text-right">{{ number_format( kinerjaDokter($d->id, $r->id, 'TA', $tga, $tgb, $cara_bayar_id) ) }}</td>
                    @endforeach --}}
