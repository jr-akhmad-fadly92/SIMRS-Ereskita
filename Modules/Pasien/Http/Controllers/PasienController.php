...

{
...

    {
      if(strtolower(Auth::user()->role()->first()->name)=='dokter')
      {
        ...
        $pasien = Pasien::join('registrasis','registrasis.pasien_id','=','pasiens.id')->where('registrasis.dokter_id',Auth::user()->pegawai_id)->groupBy('registrasis.pasien_id')
        ->select([
     ...
     return DataTables::of($pasien)
      ...
      ->editColumn('nama', function ($pasien) {
        ...
        return $pasien->nama_pasien.' BINTI '.$pasien->ibu_kandung;
    ...
    {
      if(strtolower(Auth::user()->role()->first()->name)=='administrator' ||strtolower(Auth::user()->role()->first()->name)=='rekammedis' )
      {
